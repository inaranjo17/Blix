...
exports.processSensorEvent = processSensorEvent;
const db_1 = require("../configs/db");
const socket_1 = require("../configs/socket");
const email_service_1 = require("./email.service");
const generateCode_1 = require("../utils/generateCode");
async function processSensorEvent(tableId, detected) {
    const now = new Date();
    ...
    const pendingReservation = table.reservations.find((r) => r.status === 'PENDING');
    const activeReservation = table.reservations.find((r) => r.status === 'ACTIVE');
    ...
    if (detected && pendingReservation) {
        ...
        if (secondsWaiting < 180) {
            ...
            (0, socket_1.getIO)().to('admin-room').emit('table:conflict', { tableId });
    ...
    if (!detected && table.state === 'OCCUPIED_NO_RESERVATION') {
        ...
        (0, socket_1.getIO)().emit('table:state_changed', { tableId, newState: 'FREE' });
        return { tableId, detected, state: 'FREE' };
    }
    return { tableId, detected, state: table.state };
}
// Función interna: busca mesa alternativa y reasigna
async function tryReassignment(reservation, conflictTableId, now) {
    const originalTable = await db_1.prisma.table.findUnique({
        where: { id: conflictTableId },
    });
    if (!originalTable)
        return;
    // Buscar mesa libre del mismo tamaño en zona más cercana
    const alternative = await db_1.prisma.table.findFirst({
        where: {
    ...
    if (alternative) {
        ...
        const newCode = (0, generateCode_1.generateReservationCode)(alternative.id);
        ...
        await (0, email_service_1.sendConflictReassignedEmail)(reservation.user.email, reservation.user.name, conflictTableId, alternative.id, newCode);
    }
